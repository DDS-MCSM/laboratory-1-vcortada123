...
Además, también nos permite establecer puntos de interrupción en cualquier punto de nuestro programa para que podamos pausar la ejecución en cualquier momento e inspeccionar el entorno en ese preciso instante de la ejecución. 

- _Prueba el debugger cuando se ejecuta una funció incorrecta, `bad_func()` del archivo r_sample.R, la cual tiene error de ejecución._


```{r debug_func, echo=TRUE,eval=FALSE}
...
Una vez visto cómo usar el debugger para depurar cualquier error en el código, podemos también aprovecharlo para detener la ejecución en cualquier línea insertando los breakpoints correspondientes.

- Prueba establecer unos cuantos breakpoints en cualquiera de las funciones proporcionadas. A continuación, ejecutarlas y comprueba como se detiene la ejecución en los breakpoints insertados._
...
- _A continuación, abre un terminal dentro de la carpeta del proyecto. Ejecute la orden de git para establecer la dirección del repositorio remoto_
